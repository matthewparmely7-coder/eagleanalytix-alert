const { getSeriesModel, getWatchlistModel } = require("services/db");
const { evaluateLastShow, settings, version } = require("../patterns/lastShowDemand");
const { hash } = require("./seriesGrouping");
const interval = () => Math.max(Number(process.env.LAST_SHOW_RECHECK_MS) || 900000, 1000);
...
async function evaluateSeriesBatch(batchSize = 10, check = () => {}) {
    const series = getSeriesModel(), histories = getWatchlistModel(), now = Date.now(), cfg = settings(), rule = version(cfg);
    ...
    for (const group of groups) {
        ...
        const result=evaluateLastShow(group,new Map(rows.map(r=>[r.eventID,r])),now,cfg);
...
module.exports={evaluateSeriesBatch};
